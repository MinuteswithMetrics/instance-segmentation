...
class BatchNorm(BatchNormalization):
    """
    Original BatchNorm subclass
    If the batch size is small, the accuracy drops, so do not learn
    """
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    positive_class_ids = tf.cast(
        tf.gather(target_class_ids, positive_ix),
        tf.int64)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        outputs = [Concatenate(axis=1, name=n)(list(o)) for o, n in zip(outputs, output_names)]
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                target_rois = Lambda(lambda x: K.cast(x, tf.float32) / image_scale[:4])(input_rois)
            else:
            ...
            h, w = config.IMAGE_SHAPE[:2]
            detection_boxes = Lambda(lambda x: x[..., :4] / np.array([h, w, h, w]))(detections)
